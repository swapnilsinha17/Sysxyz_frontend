...
const ManagerTaskView = () => {
  ...
  const handleMarkAsComplete = async () => {
    try {
      await axios.put(
        `${apis.baseUrl}/tasks/markAsComplete/${id}`,
        { comment, image },
        {
          headers: {
            Authorization: sessionStorage.getItem("auth_token"),
      ...
      {loading ? (
        ...
        <>
          <Box>
            ...
            <Typography variant="body1">Type: {task?.task_type}</Typography>
            <Typography variant="body1">Assigned to: {task?.assign_to}</Typography>
            <Typography variant="body1">Due Date: {task?.due_date}</Typography>
            {/* <Typography variant="body1">Extra Date: {task?.extra_fields}</Typography> */}
          </Box>
